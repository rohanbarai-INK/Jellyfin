...
const getExpiryDateText = (expiryDate?: string | null, status?: string | null) => {
    if (!expiryDate) {
        return 'Expiry Date: Not set';
    }

    const date = new Date(expiryDate);
    const formattedDate = Number.isNaN(date.getTime()) ? expiryDate : date.toLocaleDateString();
    const statusText = status ? ` (${status})` : '';

    return `Expiry Date: ${formattedDate}${statusText}`;
};
...
const UserCardBox: FunctionComponent<IProps> = ({ user = {} }: IProps) => {
    ...
    const expiryDate = getExpiryDateText(subscriptionUser.ExpiryDate, subscriptionUser.Status);
