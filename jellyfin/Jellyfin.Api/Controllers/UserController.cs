...
public class UserController : BaseJellyfinApiController
{
    ...
    public async Task<ActionResult<UserDto>> CreateUserByName([FromBody, Required] CreateUserByName request)
    {
        ...
        if (request.InitialSubscriptionDurationMonths.HasValue)
        {
            var duration = request.InitialSubscriptionDurationMonths.Value;
            if (!_supportedSubscriptionDurations.Contains(duration))
            {
                return BadRequest("InitialSubscriptionDurationMonths must be one of: 1, 3, 6, 12.");
            }

            newUser.ExpiryDate = DateTime.UtcNow.AddMonths(duration);
